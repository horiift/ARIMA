...
#
# Novo ajuste (pedido atual):
# - Considerar somente as janelas: 7, 14, 28, 60, 120, 360, 720
#   (remover 30 e incluir 28)
# ------------------------------------------------------------

# Janelas (window_days) permitidas
WINDOW_DAYS_ALLOWED = {7, 14, 28, 60, 120, 360, 720}
...
def render_period(df, daily_horizons, retrain_horizons, period_start, period_end, label, out_dir):
    ...
    # Filtrar somente as janelas permitidas
    dsel = dsel[dsel["window_days"].isin(WINDOW_DAYS_ALLOWED)]
    if dsel.empty:
        print(f"[WARN] {label}: nenhum dado nas janelas permitidas {sorted(WINDOW_DAYS_ALLOWED)}.")
        return
...
def main():
    ...
    # Filtrar somente as janelas permitidas no dataset base (garante consistência)
    df = df[df["window_days"].isin(WINDOW_DAYS_ALLOWED)]
    if df.empty:
        print(f"[ERROR] Nenhum dado nas janelas permitidas {sorted(WINDOW_DAYS_ALLOWED)} no arquivo de entrada.")
        sys.exit(1)
